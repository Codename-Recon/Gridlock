...
func load_release_data(release_data: Dictionary, version: Version) -> void:
	var notification_title : String = tr("VERSION_NOTIFICATION_TITLE", "Notification title for a new version").format({version=release_data.get("tag_name")})
	var open: ButtonToUri = ButtonToUri.new()
	open.text = tr("VERSION_NOTIFICATION_OPEN", "Open release in the browser, button in notification")
