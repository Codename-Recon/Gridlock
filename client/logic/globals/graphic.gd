...
enum AntiAliasingQuality { LOW, MEDIUM, HIGH, ULTRA }
enum ShadowQuality { LOW, MEDIUM, HIGH, ULTRA }

var msaa_quality_map: Dictionary = {
	AntiAliasingQuality.LOW: 0,
	AntiAliasingQuality.MEDIUM: 1,
	AntiAliasingQuality.HIGH: 2,
	AntiAliasingQuality.ULTRA: 3
}
var shadow_quality_map: Dictionary = {
...
func set_anti_aliasing_quality(viewport: SubViewport) -> void:
	...
	viewport.msaa_3d = msaa_quality_map[anti_aliasing]
...
func _set_graphics() -> void:
	# TODO: This has no effect atm. It gets set, but there is no graphicall difference.
	#		Maybe there is an additional process needed which applies those settings.
	var shadow_quality: int = ProjectSettings.get_setting("game/graphics_shadow_quality")
	ProjectSettings.set_setting(
		"rendering/shadows/directional_shadow/size", shadow_quality_map[shadow_quality]
	)
	ProjectSettings.set_setting(
		"rendering/shadows/directional_shadow/size", shadow_quality_map[shadow_quality]
	)
