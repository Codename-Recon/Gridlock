...
func _ready() -> void:
	hide()
	($VBoxContainer/ButtonMove as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.MOVE))
	($VBoxContainer/ButtonAttack as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.ATTACK))
	($VBoxContainer/ButtonEnter as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.ENTER))
	($VBoxContainer/ButtonDeploy as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.DEPLOY))
	($VBoxContainer/ButtonJoin as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.JOIN))
	($VBoxContainer/ButtonSupply as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.REFILL))
	($VBoxContainer/ButtonCapture as Button).pressed.connect(_button_pressed.bind(GameConst.Actions.CAPTURE))
