	...
	set(value):
		...
		if ammo > -1:
			...
			if is_inside_tree():
				...
				if ammo < properties.ammo * ProjectSettings.get_setting("global/unit_ammo_blink_threshold"):
					ammo_low = true
				else:
					ammo_low = false
	...
	set(value):
		...
		if fuel > -1:
			...
			if is_inside_tree():
				...
				if fuel < properties.fuel * ProjectSettings.get_setting("global/unit_fuel_blink_threshold"):
					fuel_low = true
				else:
					fuel_low = false
