...
func nakama_get_username(user_id: String = "") -> String:
	if user_id:
		var users: NakamaAPI.ApiUsers = await nakama_client.get_users_async(nakama_session, PackedStringArray([user_id]))
...
func nakama_set_username(user_name: String) -> void:
	var _update: NakamaAsyncResult = await nakama_client.update_account_async(nakama_session, null, user_name)
...
func nakama_create_match() -> String:
	...
	var data: Dictionary = {match_name = "%s" % nick_name, map_name = map_name, map_path = _global.selected_map.resource_path}
	nakama_match = await nakama_client.rpc_async(nakama_session, "create_match_rpc", JSON.stringify(data))
	presences = {}
...
func nakama_get_matches_list() -> NakamaAPI.ApiMatchList:
	...
	var label: String = ""
	var query: String = ""
	return await nakama_client.list_matches_async(nakama_session, player_min, player_max, player_limit, authoritative, label, query)
